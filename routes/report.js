...
function sumAmount(items) {
  return (Array.isArray(items) ? items : []).reduce((sum, item) => sum + Number(item?.amount || 0), 0);
}

function buildJsonFallback(project, expenses, tasks, photos) {
  ...
  return {
    ...
    data: {
      ...
      summary: {
        ...
        totalSpent: sumAmount(safeExpenses),
...
router.get("/:projectId", async (req, res) => {
  ...
  try {
    ...
    const totalSpent = sumAmount(safeExpenses);
    ...
    try {
      ...
      safeTasks.slice(0, 10).forEach((task, index) => {
        doc.text(`${index + 1}. ${task.title || "Untitled"} | ${task.status || "pending"} | ${task.priority || "medium"}`);
      });

      doc.moveDown(0.5);
      doc.fontSize(14).text("Expense Summary", { underline: true });
      safeExpenses.slice(0, 10).forEach((expense, index) => {
        doc.fontSize(11).text(`${index + 1}. ${expense.category || "uncategorized"} - INR ${Number(expense.amount || 0).toLocaleString()} (${expense.expense_date || "N/A"})`);
      });
      ...
      if (safePhotos.length === 0) {
        doc.fontSize(10).fillColor("#64748B").text("No photos available for this project.");
      ...
      if (!res.headersSent) {
        return res.status(200).json(buildJsonFallback(project, safeExpenses, safeTasks, safePhotos));
      }
    }
  } catch (error) {
